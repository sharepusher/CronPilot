...
命令（均自动选择 Python 3.8–3.11，无需 export PY）:
  start          启动本地开发服务 (127.0.0.1:5001)
  install        创建/更新虚拟环境并安装核心依赖
  test           运行 P0 单元测试
  check          列出本机可用 Python 版本
  python         打印将使用的 Python 与 venv 路径
  exec <args...> 用项目 venv 执行命令，如: cronpilot.sh exec python scripts/hash_login_password.py 'pwd'
...
case "$cmd" in
  start)
    exec bash "$ROOT/scripts/start_local.sh"
    ;;
  install)
