...
v2.1.0 2026-07-20 · Framework Generation（Flask 2.3 + SA 2.0）
 | 
...
- **D3-1** 已随 **v2.1.0** 交付（`assert_framework_pins`）；**D3-2 compose 实测**与 **D3-3** 仍待 Docker 环境恢复后补跑。
- 设计稿：[Phase D3 · Docker pin 矩阵](PhaseD3-Docker-pin矩阵设计.html)。

## [2.1.0] — 2026-07-20 · Framework Generation（Flask 2.3 + SA 2.0）

依赖轨 **OPT-P2-11** 绿场闭环：去 `records` → Query Contract / Repo / AST 门禁 → DEC-008 同窗 pin（Flask **2.3.3** + SQLAlchemy **2.0.36** + FSA **3.1.1**）→ Mapped[] → ensure 空库重放。Python **3.8–3.11**。无强制 Alembic 树；Tier 3c 搁置；Phase D3 compose 仍待收尾。
...
| ORM | `paginate_select` + 薄 Repo；AST 禁止 Legacy query/paginate |
| 模型 | 九表 `Mapped[]`；`test_mapped_model_guard` |
| Schema | 主路径 `ensure_business_tables`；空库重放单测 |
| 工程 | `assert_framework_pins`（D3-1）；设计先行 + 编号规范 |

### Tier 3 前置 · 去 `records`

- 调度器 `update_cron_info` 改 ORM；`scheduler_db.fetch_apscheduler_job_ids`；移除 `records`。

### Phase A / B / C

- A：`pagination.py` + 7 列表迁 `paginate_select`。
- B：`app/repositories/`；列表迁出 views。
- C：`test_orm_legacy_guard` + CI C-CI-A。设计：[Phase C](PhaseC-ORM-Legacy-AST门禁设计.html)。

### Phase D0 / D1 / D2 / Tier 3b / D3-1

- D0 DEC-008：见 [D0](PhaseD0-Framework-Generation决策.html)。
- D1 同窗 pin（含原 Tier 4 Flask 2）；D2 Mapped[]（[D2](PhaseD2-Mapped模型迁移.html)）；`cron_do` 防 detach。
- 3b-A：ensure 契约 + `TestEnsureEmptyDbReplay`（[Tier 3b](Tier3b-迁移重放与残余收束设计.html)）。
- D3-1：`assert_framework_pins`；compose 实测仍属 Unreleased。

### 未纳入 / 搁置

- Tier 3c（无存量升级）；Flask 3 / 默认 3.12+ / Alembic 基线；功能 OPT-P1-\*。

升级：更新 venv → `ensure_business_tables` → **重启**。无破坏性表结构变更。

### 协作规范

- 设计先行；编号见 [需求编号与缩写规范](需求编号与缩写规范.html)。
