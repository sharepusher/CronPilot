...
| 端口 | 宿主机 **5860** 未被占用 |
| 运行时栈 | 镜像内 **Python 3.10 + gunicorn 22.0.0 + gevent 23.9.1**（勿期望 3.12+；Tier 2 已交付，见 [依赖升级 RFC](../deps/依赖升级RFC.html)） |
...
`docker_start.sh`、`docker_gun.py` 为旧 Supervisor 方案遗留，**新镜像不再使用**。
...
- [依赖升级 RFC](../deps/依赖升级RFC.html) — Tier 0～4 分层路线、Docker/Python 栈约束
