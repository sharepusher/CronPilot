...
case "$cmd" in
  ...
  test)
    ...
    "$CRONPILOT_VENV/bin/python" -m unittest tests.test_p0_phase_a tests.test_cronpilot_sign tests.test_job_log_display tests.test_job_log_outcome tests.test_check_conf_production tests.test_rbac_phase -v
    ;;
  check)
