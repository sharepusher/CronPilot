...
> HTML 版：[Docker部署指南.html](Docker部署指南.html) · [文档索引](index.html) · 裸机部署：[非Docker部署指南.md](非Docker部署指南.md)
...
验证 **裸机安装脚本**（非运行镜像）在 Ubuntu / Rocky8 / CentOS7 容器内是否通过：
...
| 容器启动后 502 / 无响应 | `docker compose logs`；确认 `conf.ini` 已挂载且为文件 |
...
- [非 Docker 部署指南](非Docker部署指南.md) — 裸机生产、systemd、Nginx
- [ubuntu安装与运行.md](ubuntu安装与运行.md) — Ubuntu 裸机分步
- [linux安装与运行.md](linux安装与运行.md) — Linux 安装总览
