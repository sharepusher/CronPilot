...
表 `operation_log`；服务 `operation_log_service`；路由 `/operation_log_list`（权限 `operation:read`，operator+admin；viewer 不可见）；RBAC「审计」仍为 `audit:read`（仅 admin）。写入 create/update/toggle/retire；配置 `operation_log_counts`。详 [架构 §6.5](../arch/架构设计文档.html#op-audit-detail) · [RBAC §3.2.1](../design/RBAC架构设计方案.html)。
...
**与规模化 IA：**任务级健康预计算与 L1 指标条见 [OPT-P2-13](../design/规模化信息架构设计.html)（B0 `job_health`）；本项图表/趋势对应 OPT-P2-13 的 B2+。
...
**验收：**见 [RBAC v4 详设](../design/RBAC架构设计方案.html) · [落地路线](../design/RBAC落地路线.html) · [交付状态](../交付状态与路线图.html)。已打 `git tag v1.0.0`。
...
权威文档：[依赖升级 RFC v1.1](../deps/依赖升级RFC.html)。与 OPT-P2-10（RBAC）关系：RBAC 在 Tier 0 后可启动，**不必**等待 gevent / Flask 2。
...
完整状态与版本见 [**交付状态与路线图**](../交付状态与路线图.html)。

| ID | 名称 | 优先级 | 状态 | 版本 |
| --- | --- | --- | --- | --- |
| OPT-P0-01 | SQL 参数化 | P0 | 已交付 | v0.1.0 |
| OPT-P0-02 | 密码哈希 | P0 | 已交付 | v0.1.0 |
...
| OPT-P1-03 | 执行详情页 | P1 | 设计待确认 | — |
| OPT-P1-04 | 立即执行 | P1 | 设计待确认 | — |
| OPT-P1-05 | 强化 add\_log | P1 | 未开始 | — |
