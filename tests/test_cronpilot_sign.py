...
class TestCronpilotSign(unittest.TestCase):
    def test_sign_stable(self):
        data = {'cronpilot_trace_id': 'abc-123', 'foo': 'bar'}
        s1 = get_cronpilot_sign(data, api_key='testkey')
        s2 = get_cronpilot_sign(data, api_key='testkey')
        self.assertEqual(s1, s2)
        self.assertEqual(len(s1), 32)

    def test_sign_changes_with_trace_id(self):
        k = 'key'
        a = get_cronpilot_sign({'cronpilot_trace_id': '1'}, api_key=k)
        b = get_cronpilot_sign({'cronpilot_trace_id': '2'}, api_key=k)
