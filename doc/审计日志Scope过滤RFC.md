...
编号：OPT-P2-13/AUDIT-SCOPE · 日期：2026-07-31 · 状态：**已交付**（方案 A · 按操作者组过滤）  
前置依赖：OPT-P2-12（管理员 admin Scope 差异化）已交付

## 一、问题

管理员 admin Scope 差异化（OPT-P2-12）已交付后，按组管理员在任务列表、执行日志、操作记录、用户管理等页面均已按业务组隔离。
...
2. 按组管理员对 scope 外的用户**既不可见也不可操作**（OPT-P2-12 已落地），因此跨组操作的审计记录实际上不存在。
