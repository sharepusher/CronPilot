...
- **业务组下拉文案优化**：「全部（可见）」→「全部业务组（可见）」，语义更明确
- **"调度异常"按钮样式统一**：未选中时与其他筛选按钮样式一致，仅选中时带 orphan 颜色；移除未选中态的特殊边框和背景

### 修复 — v2 用户查看页无条件展示"停用原因"

- **问题**：v2 redesign 用户查看页对所有用户（含启用状态）均展示"停用原因"行，对启用用户显示"—（未填写）"造成困惑
- **修复**：`user_form.html` view mode 中"停用原因"行用 `{% if not user.is_active %}` 包裹，仅对已停用用户展示
- **对照**：v1 模板（`users_edit.html`）本就正确处理此逻辑，v2 重构时未对齐
...
- IP 维度：50 次失败 / 15 分钟窗口 → 锁定 5 分钟（原 5 次 / 5 分钟 → 锁定 15 分钟）
- 用户名维度：50 次失败 / 30 分钟窗口 → 锁定 10 分钟（原 10 次 / 10 分钟 → 锁定 30 分钟）
- 适用场景：开发/测试环境频繁登录、多人共享 IP 办公环境
- 测试 `test_login_limiter` 同步更新滑动窗口断言值
